    ...
    <Header />
      ...
      <Container>
        <Row className="my-5 py-5">
          <Col md={12}>
            {(apartment.length === 0) ? <img className='img-fluid w-30 loader-section' src={loader}/> : <img className='img-fluid w-100' src={`data:image/jpeg;base64,${apartment[0]?.image.img}`} alt={apartment[0]?.title} />}
            <div className="title-price overflow-hidden mt-5">
             <h3 className="float-left">House : {apartment[0]?.title}</h3>
             <h3 className="float-right">Price : ${apartment[0]?.price}</h3>
            </div>
